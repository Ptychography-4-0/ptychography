from typing import TYPE_CHECKING, Tuple, Optional, NamedTuple, List, Union
...
class GridSpec:
    ...
    def min_patch_index(self):
        ...
        assert candidates
        min_index = np.iinfo(np.dtype(np.int32)).max
        for c in candidates:
            min_index = np.minimum(min_index, c.patch_index)
        return min_index
...
class PatchWDDUDF(UDF):
    ...
    This approach allows much better [O(N)?] scaling for large reconstruction areas. The
    method is experimental and not fully validated.
    ...
    recon_parameters
        Reconstruction plan generated by :func:`ptychography40.reconstruction.wdd.\
            params_recon.wdd_params_recon`
    patch_overlap
        (y, x) overlap of reconstruction patches after subtracting trim, default is two
    patch_trim
        Remove border pixels of patches to suppress edge effects, default 2 pixel
    complex_dtype
        Complex dtype for array
    ...
    def __init__(self, recon_parameters: WDDReconstructionPlan,
                 patch_overlap: Optional[Tuple] = None,
                 patch_trim: Optional[Union[Tuple, int]] = None,
                 complex_dtype: "nt.DTypeLike" = np.complex64):
