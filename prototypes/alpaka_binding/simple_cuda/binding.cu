...
PYBIND11_MODULE(cuBinding, m) {
  decare_algo_binding<CuAlgo<float, int>>(m, "CuAlgoFI");
  decare_algo_binding<CuUmemAlgo<float, int>>(m, "CuUmemAlgoFI");
}
